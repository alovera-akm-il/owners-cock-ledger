...
| Punishment assigned (incl. via escalation) | Submissive | `punishment.assigned` | Yes |
| Punishment deadline approaching (§3 in `08-punishments-and-deadlines.md`) | Submissive | `punishment.deadline_approaching` | Yes |
| Punishment auto-failed (deadline) or judged-failed | Both | `punishment.failed` | Yes |
| Punishment completion proof submitted | Keyholder | `punishment.proof_submitted` | Yes |
| Punishment completed/revoked | Submissive | `punishment.resolved` | No — informational, feed is enough |
| Reward given | Submissive | `reward.given` | Yes (this one's meant to feel good in the moment) |
| Confinement timer adjusted (manual or punishment-driven) | Submissive | `confinement.adjusted` | Yes if it's an *extension*; feed-only if it's a reduction (good news travels fine without an interrupt) |
| Time-extension punishment applied **via automatic escalation** (`assigned_via='system'`, `01-data-model.md` §4's `keyholder_reviewed_at` starts NULL) | Keyholder | `confinement.time_extension_needs_review` | Yes — this is the one notification in the whole matrix that exists specifically to get a human to look at something the system just did on its own; see `08-punishments-and-deadlines.md` §6. Not sent for a manually-assigned time-extension punishment, since the Keyholder already saw the amount before confirming it themselves. |
...
once designed) — the pattern (event → row → optional push) doesn't
change per-feature.
