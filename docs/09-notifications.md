...
| Lock timer resumed (release date just moved forward by the pause length) | Submissive | `confinement.clocks_resumed` | Yes — resuming changes a real number (their displayed release date), not just a status label, per `08-punishments-and-deadlines.md` §9. Scoped to the lock timer only — task deadlines are unaffected, see that section for why. |
