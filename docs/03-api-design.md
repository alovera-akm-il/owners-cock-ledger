...
| `POST /auth/password-reset/request` | public | `{email}` → **always** returns the same generic `202` ("if that email has an account, a reset link is on its way") regardless of whether the email exists, whether the deployer has SMTP configured at all, or whether the send actually succeeds — none of that is ever visible to the caller. The lookup, token creation, and email dispatch happen in a spawned background task *after* responding, not before, so response timing can't leak account existence the way an "only look up if worth it" implementation would (`05-security-and-privacy.md` §11 has the full reasoning). Rate-limited per-IP and per-email, same posture as `POST /auth/login`'s brute-force limiter (`05-security-and-privacy.md` §2). No-ops (still returns the same `202`) if the deployer hasn't configured outbound email — the endpoint's existence doesn't imply the capability is actually on. |
| `POST /auth/password-reset/redeem` | public (requires a valid, unexpired, unconsumed `password_reset_tokens` row) | `{token, new_password}` → sets `password_hash`, consumes the token, and revokes every other existing session for this account in the same transaction, same behavior as `POST /auth/password/change` above. Redeems a token from either issuance path — `owners-cock-ledger admin reset-password <email>` (`10-operations.md` §5) or the self-service request above — identically; the endpoint has no way to know or care which one created the row it's consuming. |
