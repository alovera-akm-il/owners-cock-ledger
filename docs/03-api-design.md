...
| `POST /auth/password-reset/redeem` | public (requires a valid, unexpired, unconsumed `password_reset_tokens` row) | `{token, new_password}` → sets `password_hash`, consumes the token, and revokes every other existing session for this account in the same transaction, same behavior as `POST /auth/password/change` above. The token itself is never issued by any endpoint — only by `owners-cock-ledger admin reset-password <email>` (`10-operations.md` §5) — so there is no companion "request a reset" route, deliberately: with no outbound email system, one would just be a way to probe which emails have accounts. |
