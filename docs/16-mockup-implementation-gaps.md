...
| `submit-proof.html` | `submit_proof.html` (`/submissive/submit-proof`) + `assignment_proof.html` (task-specific) | Deep | See item 4: no live code-expiry countdown, no in-page "request new code," no in-browser record capture (upload-only). Voice as a proof `kind` is correctly *absent* from `submit_proof.html`'s selector (verification-code proof is photo/video only per `04-verification-workflow.md`; voice is real and correctly present on the task-specific `assignment_proof.html` instead, driven by `media_options`). |
